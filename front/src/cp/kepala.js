...
export default function Kepala(props){
	let classes=useStyle()
		let {conf}=props
		return <AppBar color='default' elevation={0} className={classes.appBar}>
			<Toolbar className={classes.toolbar}>
				...
				<nav>
				{conf.menune.map(i1=>{
					return<Link component={RouterLink} to={i1.menuju} color='textPrimary' variant='button'
 className={classes.link}>{i1.teks}</Link>
				})}
				</nav>
			</Toolbar>
		</AppBar>
